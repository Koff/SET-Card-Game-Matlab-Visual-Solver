...
I2 = imcrop(IMG,[62 618 1750 1500]); % Cropping
...
parfor i=1:12
	...
	visualLocal = visual( card(i).yi:(card(i).yf-3) , card(i).xi:(card(i).xf-3)  ,:);
	...
	[x,y] = find(L==1); % Coordinates of the pixels in white (in the color image, with color)
	temporal = visualLocal;
    ...
    if ( find(RGB == max(RGB))==1 )
      card(i).Color = 'Red';
      if ( abs(RGB(1)-RGB(3)) < 12 )  % Red and blue are close colors in the pics, this is to correct some of the mis classifications
			card(i).Color = 'Blue';
		end
	end
